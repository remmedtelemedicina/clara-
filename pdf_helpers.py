...
S = _S  # exported alias so wildcard imports can access it

TH  = _S('th',  fontName='Helvetica-Bold',        fontSize=8,   leading=10, textColor=WHITE)
TD  = _S('td',  fontName='Helvetica',              fontSize=8,   leading=10, textColor=BLACK)
TDB = _S('tdb', fontName='Helvetica-Bold',         fontSize=8,   leading=10, textColor=BLACK)
BOX = _S('box', fontName='Helvetica',              fontSize=8,   leading=11, textColor=BLACK)
BODY= _S('bod', fontName='Helvetica',              fontSize=8.5, leading=12, textColor=BLACK, alignment=TA_JUSTIFY)
BODL= _S('bdl', fontName='Helvetica',              fontSize=8.5, leading=12, textColor=BLACK)
H2S = _S('h2s', fontName='Helvetica-Bold',         fontSize=10,  leading=13, textColor=NAVY)
H3S = _S('h3s', fontName='Helvetica-Bold',         fontSize=9,   leading=12, textColor=NAVY)
FOOT= _S('ft',  fontName='Helvetica',              fontSize=7,   leading=9,  textColor=colors.HexColor('#444444'))
...
def bul(t, i=12): return Paragraph(f'• {t}', _S('bl', fontName='Helvetica', fontSize=8.5, leading=12, leftIndent=i))
def arr(t, i=15): return Paragraph(f'→ {t}', _S('ar', fontName='Helvetica', fontSize=8.5, leading=12, leftIndent=i))

def _hex(c):
    return f'{int(c.red*255):02X}{int(c.green*255):02X}{int(c.blue*255):02X}'

def sec_hdr(title, bg=NAVY):
    pp = Paragraph(title, _S('sh', fontName='Helvetica-Bold', fontSize=11, leading=15, textColor=WHITE))
...
def info_box(title, lines, bc=NAVY, bg=LT_BLUE, tc=None, italic_title=True):
    ...
    items = [Paragraph(f'<font color="#{hx}">■ </font><%s>%s</%s>' % (fmt,title,fmt),
                       _S('it', fontName='Helvetica-BoldOblique', fontSize=8.5, leading=12, textColor=tc))]
    for ln in lines:
...
def gab(ltr, lines):    return info_box(f'Gabarito: {ltr}', lines, bc=DK_GRN, bg=LT_GRN, tc=DK_GRN)

def checklist(title, items_list):
    hx = _hex(DK_GRN)
    rows = [Paragraph(f'<font color="#{hx}">■ </font><b>{title}</b>',
                      _S('cbt', fontName='Helvetica-Bold', fontSize=8.5, leading=12, textColor=DK_GRN))]
...
def q_block(label, qtext, opts, gab_ltr, comment):
    ...
    out.append(Paragraph(f'<b>{label}: {qtext}</b>',
                         _S('qt', fontName='Helvetica-Bold', fontSize=8.5, leading=12)))
    out.append(sp(1))
    for o in opts:
        out.append(Paragraph(o, _S('qo', fontName='Helvetica', fontSize=8.5, leading=12, leftIndent=10)))
    out.append(sp(1))
...
def ni(num, bold, expl, i=15):
    return [
        Paragraph(f'<b>{num}. {bold}</b>', _S('ni', fontName='Helvetica-Bold', fontSize=8.5, leading=12)),
        Paragraph(f'→ {expl}', _S('ne', fontName='Helvetica', fontSize=8.5, leading=12, leftIndent=i)),
    ]
