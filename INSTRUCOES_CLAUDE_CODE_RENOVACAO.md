# 📋 INSTRUÇÕES PARA O CLAUDE CODE — Atualização do Fluxo de Renovação da Clara

> **Documento gerado em:** 15/06/2026
> **Repositório:** `remmedtelemedicina/clara-`
> **Branch:** `claude/print-formatting-fix-7DleV`
> **Arquivo-fonte:** `prompt_renovacao.md`
...
1. **INSTRUÇÕES** — o que fazer e como aplicar o prompt no n8n / Claude Code
2. **PROMPT COMPLETO** — o texto integral e já corrigido do fluxo de renovação de receitas e exames da Clara (colar no nó da IA no n8n, ou ler de arquivo)

---

## 🚀 COMO USAR NO CLAUDE CODE

Se você quer que o **Claude Code** aplique ou revise este prompt, copie e cole o comando abaixo:

```
Leia o arquivo prompt_renovacao.md deste repositório. Ele é o prompt completo
da Clara (IA de atendimento da REMMED) para renovação de receitas e exames.

Quero que você:
1. Revise o fluxo inteiro procurando erros de lógica, estados órfãos,
   contradições entre seções e caminhos que não levam a lugar nenhum.
2. Confira se TODA a lista de antidepressivos C1 da CLASSIFICAÇÃO aparece
   também nos caminhos de "mais de 6 meses" (Saúde Mental R$249).
3. Verifique se cada setor do MAPA DE ETAPAS tem um bloco correspondente.
4. Garanta que nenhuma regra de negócio foi quebrada (receita azul/amarela
   não oferece consulta; antibiótico oferece; emagrecimento vai pra Controle
   de Peso; biológicos e insulinas têm tratamento próprio).
5. Liste os problemas encontrados e corrija direto no arquivo.

Depois faça commit e push na branch claude/print-formatting-fix-7DleV.
```

---

## 🛠️ COMO COLOCAR NO n8n (produção)

A Clara lê este prompt no nó da IA. Como o repositório é **privado** e o n8n roda em **Docker**, o caminho recomendado é ler de uma **pasta montada como volume**:

### 1) No servidor, crie a pasta e coloque o arquivo
```bash
mkdir -p /home/n8n/prompts
# copie o prompt_renovacao.md para dentro dela
```

### 2) Monte a pasta como volume no `docker-compose.yml`
```yaml
services:
  n8n:
    volumes:
      - n8n_data:/home/node/.n8n          # já deve existir
      - /home/n8n/prompts:/data/prompts    # ← ADICIONE ESTA
```
...
### 4) No n8n, monte o fluxo de leitura
```
[Trigger] → [Read/Write Files from Disk] → [Extract from File] → [IA]
```
- **Read/Write Files from Disk** → Operation: `Read File(s) From Disk` → Selector: `/data/prompts/prompt_renovacao.md`
- **Extract from File** → converte o binário em texto → cai em `{{ $json.data }}`
- **Nó da IA** → no system prompt usa: `{{ $json.data }}`

> ⚠️ **Atenção:** quando você atualizar o prompt, copie o arquivo novo para `/home/n8n/prompts/` de novo — senão o n8n continua lendo a versão antiga (não precisa reiniciar, só trocar o arquivo).

### Alternativa — colar texto direto
Se preferir colar na mão: o prompt cabe em **1 mensagem** do WhatsApp/Central de Comandos (~64 mil caracteres, limite é 65.536). Cole o prompt sozinho numa mensagem e o comando em outra — se juntar texto extra, o WhatsApp corta o final.
...
| # | Melhoria | Impacto |
|---|----------|---------|
| 1 | **Lista de medicamentos expandida** — de 7 para 16 categorias, com genéricos + nomes comerciais | Clara para de errar a classificação e de mandar caso pra humano à toa |
| 2 | **Antidepressivos C1 completos** em TODOS os caminhos (inclusive "mais de 6 meses") | Corrige o Bug do caso Rafaela: Mirtazapina/Paroxetina vencida agora vai pra Saúde Mental R$249, não clínico R$79,90 |
| 3 | **Regra de polipílulas** — verifica cada substância da associação | Evita renovar combinação que contém substância controlada |
| 4 | **Regra de receita magistral** (manipulado) | Renova se não tiver controlado; senão, humano |
| 5 | **Biológicos** (Humira, Enbrel, Remicade) → HUMANO | Não tenta renovar medicamento de alta complexidade |
| 6 | **Insulinas** → consulta (não renova pelo fluxo padrão) | Segurança no diabético insulino-dependente |
| 7 | **Triagem de exames** (OPÇÃO 2) com finalidade + sugestão de exames por medicamento | Clara sugere os exames certos (ex: Warfarina→INR, Metformina→HbA1c) |
| 8 | **Novas categorias**: cardiológicos, neurológicos, pulmonares, reumatológicos, urológicos, antipsicóticos, oftalmológicos, dermatológicos | Cobertura muito maior de casos reais |

---

## ⚠️ REGRAS DE NEGÓCIO QUE NÃO PODEM SER QUEBRADAS

- **Receita azul** (benzodiazepínicos): NÃO renova, NÃO oferece consulta
- **Receita amarela** (opioides, estimulantes/TDAH): NÃO renova, NÃO oferece consulta
- **Anabolizantes, Roacutan, hormônios controlados**: NÃO renova, NÃO oferece consulta
- **Antibióticos**: NÃO renova, MAS oferece consulta clínico geral R$79,90
- **Emagrecimento (GLP-1)**: NÃO renova → Consulta de Controle de Peso R$249,00
- **Antidepressivo C1 vencido (+6 meses)**: → Consulta de Saúde Mental R$249,00
- **Biológicos**: → HUMANO
- **Insulinas**: → consulta R$79,90
- **Renovação = cópia exata**: qualquer mudança de dose/remédio/forma exige nova consulta

---

═══════════════════════════════════════════════════════════════════════
# 📄 PROMPT COMPLETO (cole isto no nó da IA / arquivo)
═══════════════════════════════════════════════════════════════════════
